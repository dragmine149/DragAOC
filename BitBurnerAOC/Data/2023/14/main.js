...
function collaspe_north(grid) {
    for (let rockIndex = 0; rockIndex < rocks.length; rockIndex++) {
        let cRock = rocks[rockIndex];
        if (cRock[1] == 0) {
            // already at the highest possible point
            continue;
        }

        let filterWalls = walls.filter((v, i) => v[0] == cRock[0] && v[1] < cRock[1]);
        if (filterWalls.length == 0) {
            let filterRocks = rocks.filter((v, i) => v[0] == cRock[0] && v[1] < cRock[1]);
            if (filterRocks.length == 0) {
                cRock[1] = 0;
                continue; 
            }
        }
