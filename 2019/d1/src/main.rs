...
// Runs the calculation to get the fuel for the specified mass.
...
fn main() {
    ...
    // looks though all lines
    let mut total_fuel: f32 = 0.0;
    for line in reader.lines() {
        let mass = line.expect("Failed to read line");

        // makes sure we have some fuel to read (instead of just a blank line)
        if mass.is_empty() {
            continue;
        }
        ...
        if part == "2" {
            ...
            'fuel_loop: loop {
                // set it to itself so we can keep looping
